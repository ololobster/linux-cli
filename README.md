...
Настроить вход на сервер без пароля (ключи должны быть сгенерированны):
```
...
`>` перезаписывает файл, а `>>` добавляет содежимое в конец файла.
...
Вевести свободное место на дисках:
```
$ df -hT
```
...
Форматирать диск (указывать раздел, например, `/dev/sdc1`):
```
...
Получить из пути путь родителькой папки:
```
$ dirname ⟨path⟩
```
...
1. Если надо копировать содержимое каталога, то добаваляем `/` в конце пути.
   Если надо копировать сам каталог, то не добавляем.
1. `-r` ака `--recursive` — рекурсивно копировать папку.
1. `-l` ака `--links` — копировать символьные ссылки как симольные ссылки.
...
1. `:w` — сохранить.
1. `:q` — выйти из `vim`.
1. `:wq` — выйти из `vim` с сохранием.
1. `:q!` — выйти из `vim` без сохрания.
...
1. `Wants` — слабая версия `Requires`.
   Наш сервис будет выпоняться, даже если systemd не сумеет запустить эту зависимость.
...
Ещё раз попробовать сконфигурировать сломаные пакеты:
```
# apt-get autoremove
```
...
1. `parent` (по умолчанию) — при вызове `fork()` или `vfork()` GDB продолжит дебажить родительский процес.
