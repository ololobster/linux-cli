...
1. Регулярки:
   ...
   [[ 'fail' == f* ]]
   ```
1. Проверить существует ли файл `/etc/resolv.conf`:
   ```
   [ -f /etc/resolv.conf ]
   ```
1. Проверить, что файл `/etc/resolv.conf` существутет и его размер больше 0:
   ```
   [ -s /etc/resolv.conf ]
   ```
1. Проверить существует ли каталог `/etc`:
   ```
   [ -d /etc ]
   ```
1. Проверить существует ли переменная `ololo`:
   ```
   [ -v ololo ]
   ```
