...
x
```
Проверить наличие ключа в JSON-файле:
```
if jq -e 'has("b")' in.json > /dev/null
then
    ...
fi
```
...
Закачать что-нибудь, сохранив в указанный файл:
```
$ curl --output ⟨path⟩ ⟨url⟩
```

Оставить оригинальное имя файла, сохранить в указанный каталог:
```
$ curl --remote-name --output-dir ⟨path⟩ ⟨url⟩
```
...
1. Почти все переменные окружения отбрасываются, чтобы они не повлияли на сборку.
   Если какую-то переменную надо передать сборщику: `--preserve-envvar ⟨var⟩` или `-e⟨var⟩`.
   Если надо задать переменную для сборщика: `--set-envvar ⟨var⟩=⟨value⟩` или `-e⟨var⟩=⟨value⟩`.
...
1. Если файл `in.txt` не содержит строку:
   ...
   ```
1. Если удалённый порт открыт, таймаут 5 секунд:
   ```
   if nc -z -w5 "${HOST}" "${PORT}" &>/dev/null
   then
       ...
   fi
   ```
