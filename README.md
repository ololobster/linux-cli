...
Если не указать файл, то `sed` будет читать из stdin.
...
```
Например, избавиться от `\r`: `s/\r//g`.

Без флажка `g` будет заменять лишь 1 раз в каждой строке (1-е вхождение).
Вместо `g` можно указать номер вхождения, например, `s/⟨before⟩/⟨after⟩/2` заменит 2-е вхождение `⟨before⟩` в каждой строке.

Если `/` неудобен в качестве разделителя, то можно использовать другой символ, например, `s!/usr/lib!/usr/share!g`.

Можно указать строки, с которыми надо работать, например:
- `1` — 1-я строка (нумерация строк идёт с 1);
- `1,3` — строки с 1-й по 3-ю включительно;
- `3,$` — все строки начиная с 3-й.

Например, добавить префикс к первым 3 строкам файла:
```
$ sed -i '1,3s/^/⟨prefix⟩/' ⟨file⟩
```

Прочие действия:
1. `/⟨pattern⟩/s/⟨before⟩/⟨after⟩/g` — осуществить замену в строках, содержащих `⟨pattern⟩`.
1. `3d` — удалить 3-ю строку.
1. `/⟨pattern⟩/d` — удалить строки, содержащие `⟨pattern⟩`.
1. `3c\⟨new line⟩` — заменить 3-ю строку.
1. `1i\⟨new line⟩` — добавить строку перед 1-й строкой (т.е. в самое начало).
1. `1a\⟨new line⟩` — добавить строку после 1-й строки.

При помощи `;` можно применить несколько действий за раз:
```
...
Мандатная метка = метка безопасности = уровень конфиденциальности + категории + уровень целостности.
Классификационная метка = метка конфиденциальности = уровень конфиденциальности + категории.
...
Правила доступа:
1. Субъект может изменять объект, если Lₛ = Lₒ, Cₛ = Cₒ, iLₛ ≥ iLₒ (уровни конфиденциальности и набор категорий совпадают, а уровень целостности субъекта не ниже уровня целостности объекта).
1. Субъект может читать объект, если Lₛ ≥ Lₒ, Cₛ ≥ Cₒ.
...
Флаги:
- `ccnr` — игнорировать уровень конфиденциальности каталога при просмотре его содержимого;
...
Вывести все-все динамические библиотеки, нужные для запуска бинарника:
```
$ ldd ⟨binary file⟩
```
...
1. Проверить, что файл или каталог существует:
   ...
   [ -e /etc/resolv.conf ]
   ```
1. Проверить, что обычный файл существует:
   ```
   [ -f /etc/resolv.conf ]
   ```
1. Проверить, что обычный файл существутет и его размер больше 0:
   ...
   [ -s /etc/resolv.conf ]
   ```
1. Проверить, что каталог существует:
   ```
   [ -d /etc/X11 ]
   ```
