...
Примеры действий:
- `s/before/after/g` — осуществить замену «before» на «after»;
- `s/\r//g` — избавиться от `\r`;
- `1,3s/before/after/g` — осуществить замену «before» на «after» в строках с 1 по 3;
- `1,3s/^/prefix/` — добавить префикс в строки с 1 по 3;
- `/pattern/s/before/after/g` — осуществить замену «before» на «after» в строках, которые содержат «pattern»;
...
- `1i\new line` — добавить строку перед 1-й строкой (т.е. в самое начало);
- `1a\new line` — добавить строку после 1-й строки.

Примечания:
1. Указание строк, к которым следует применить действие: `1` — самая 1-я строка, `1,3` — строки с 1-й по 3-ю включительно, `3,$` — все строки начиная с 3-й.
...
1. Если `/` неудобен в качестве разделителя, то можно использовать другой символ, например, `s!/usr/lib!/usr/share!g`.
