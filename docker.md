...
Демон Docker (`dockerd`) занимается высокоуровневым управленимем конетйнерами.
Утилита `docker` — это клиент, отсылающим ему команды.
Среда выполения (Docker Runtime) работает на более низком уровне, непосредственно занимается изоляцией процессов, запускаемых в контейнерах.
